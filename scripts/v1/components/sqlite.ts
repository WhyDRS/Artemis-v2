...
export function dbreadidx(db: sqllite, table: string, rowIndex: number) {
    dbmaketable(db, table);
    const sql = `SELECT value FROM ${table} WHERE rowid = ?`;
    const stmt = db.prepare(sql);
    const result = stmt.get(rowIndex);
    return result;
  }

  export function dbreadnameidx(db: sqllite, table: string, rowIndex: number) {
    dbmaketable(db, table);
    const sql = `SELECT name FROM ${table} WHERE rowid = ?`;
    const stmt = db.prepare(sql);
    const result = stmt.get(rowIndex);
    return result;
  }

export function dbdelete(db: sqllite, table: string, name: string) {
  dbmaketable(db, table);
...
export function dbtablelen(db: sqllite, table: string): number {
    dbmaketable(db, table);
...
export default { dbdroptable, dbmaketable, dbopen, dbread, dbreadidx, dbreadnameidx, dbwrite, dbdelete, dbtablelen };
